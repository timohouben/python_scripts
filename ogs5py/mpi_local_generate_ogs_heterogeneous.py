...
## This script generates OGS setups for a heterogeneous, 2D, vertical model domain. Every model setup ist first generated with a steady state configuration, afterwards the OGS run starts, output is redirected into the "output folder" and input files are changed to input files with transient settings.
...
# split up the seed_list to disctribute on different slots for parallel computing
seed_list_list = []
if not int(len(seed_list)/slots):
    print("Number of realisations should be dividable by number of slots!")
for i in np.arange(len(seed_list)/slots):
    seed_list_list.append(seed_list[int(i*slots):int((i+1)*slots)])
# update the list according to the current slot (rank)
seed_list_new = seed_list_list[rank]
print(seed_list_new)
# ------------------------ogs configuration---------------------------------- #
            ...
            for mean in mean_list:
                for storage in storage_list:
                    ...
                    for recharge, rech_abv in zip(recharge_list,rech_abv_list):
                        ...
                        name=str(overall_count) + '_rank_' + str(rank) + '_var_' + str(var) + '_len_' + str(len_scale) + '_mean_' + str(mean) + '_seed_' + str(seed) + '_stor_' + str(storage) + '_rech_' + str(rech_abv)
                        output = "output"
                        dim_no = 2
