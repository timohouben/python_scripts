...
The resulting models will have different thicknesses and connection length of river and aquifer.
...
#ogs_root = "/Users/houben/PhD/ogs5/executable/ogs5_bugfix_RWPT"
...
length = 1000
thickness_list = np.linspace(1000,100,10).tolist() + np.linspace(90,10,9).tolist()
thickness_list = [int(i) for i in thickness_list]
# the relative lengths of the connection between the aquifer and the river: L_c / thickness
c_length_list = np.round(np.linspace(0.1,1,10).tolist(),2)

# -------------------- time configuration
# I prefer to work with seconds, because it is default in OGS.
time_start = 0
        ...
        for thickness in thickness_list:
            for c_length in c_length_list:
                ...
                if (overall_count - start) % slots == rank:
                    ...
                    #ogs.msh.export_mesh("/Users/houben/phd/python/scripts/ogs5py/mesh.vtk", file_format="vtu")
                    #ogs.msh.show()
                    # -------------------- GLI
                    river_low = thickness - (c_length * thickness)
                    print("#######" + str(np.round(river_low,0)))
                    ...
                    if int(river_low) != 0:
                        ogs.gli.add_points(points=(length, 0.0, np.round(river_low, 0)),
                            names="river_low")
                        ogs.gli.add_polyline(name="river", points=["river_low", "C"])
                    else:
                        ogs.gli.add_polyline(name="river", points=["B", "C"])
                    ...
                    ogs.mmp.add_block(
                        ...
                        STORAGE=[[1, storage]],
                        PERMEABILITY_TENSOR=[["ISOTROPIC", k]],
                    )
                    # -------------------- NUM
