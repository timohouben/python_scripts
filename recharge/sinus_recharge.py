...
def sinusrecharge(
    number_of_time_steps=365, time_step_size=86400, osc_duration=30, damping=0.004
):
    """
    ...
    if osc_duration <= 1 or osc_duration_str.isdigit() == False:
        print(
            "The oscillation duration must be set to a value greater than 2 and it needs to be an integer."
        )
    else:
        import numpy as np
        ...
        time = np.linspace(
            0, (number_of_time_steps) * time_step_size, number_of_time_steps + 1
        )
        ...
        mm_y = sum(
            ...
            * (365 * 86400 / (time_step_size * number_of_time_steps))
        )
