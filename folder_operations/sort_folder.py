...
integer_list_save = [int(i) for i in integer_list]
np.savetxt(directory + "/new_directories.txt", integer_list_save)
...
for i, item in enumerate(bins):
    ...
    for j, jtem in enumerate(integer_list):
        if jtem >= item[0] and jtem <= item[1]:
            os.rename(
                directory + "/" + folder_list[j], str(new_dir) + "/" + folder_list[j]
            )
