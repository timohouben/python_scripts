...
for i,curr_dir in enumerate(list_dir):
    ...
    def get_curve(path_to_project, name_of_project_ogs, curve=1, mm_d=False, time_steps=None):
        ...
        np.savetxt(str(path_to_project) + '/' + 'rfd_curve#'+ str(curve) + '.txt', rfd_y)         
...
#        return recharge
    ...
    def plot_obs_vs_time(obs_per_plot, which):
        ...
        if np.size(time_d) != np.size(recharge):
            ...
            print("Set time_steps equal to the number of time steps-1 you want to plot.")
            ...
            if which == 'min':
                ...
                for obs in obs_per_plot:
                    ...
                    head_gw_model = gethead_gw_model_each_obs(make_array_gw_model(
                            ...
                            + '/H.OUT'), index=2)), convert_obs_list_to_index(obs))
                    #ax2.plot(time_d, head_gw_model, label = str(obs) + ' GW_model', linestyle='-')
                    ax2.plot(time_d, head_gw_model, label = str(obs) + ' analytic', linestyle='-')
            elif which == 'all':
                for obs in obs_per_plot:
