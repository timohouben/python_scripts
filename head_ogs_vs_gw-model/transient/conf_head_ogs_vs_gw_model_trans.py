...
if __name__ == "__main__":
    ...
    try:
        ...
        time_s = np.loadtxt(
            path_to_project + name_of_project_gw_model + "/OutputTimes.in"
        )
...
# =============================================================================
def get_curve(
    path_to_project, name_of_project_ogs, curve=1, mm_d=True, time_steps=None
):
    """ 
    ...
    if time_steps:
        return (
            rfd_x[curves_begin[curve - 1] : (curves_begin[curve - 1] + time_steps + 1)],
            rfd_y[curves_begin[curve - 1] : (curves_begin[curve - 1] + time_steps + 1)],
        )
    else:
        return (
            rfd_x[
    ...
    try:
        ...
        if single_file == False or single_file == None:
            ...
            if which == "max":
                ...
                head_ogs_timeseries_each_obs = tecs[process][observation_point]["HEAD"][
                    :, number_of_columns - 1
                ]
            elif which == "min":
...
if __name__ == "__main__":
    rfd_x, recharge = get_curve(
        path_to_project, name_of_project_ogs, curve=1, mm_d=True, time_steps=time_steps
    )
