...
def plot():
        ...
        ax.text(obs[10], ((max(head_ogs_lower[:,0]) - min(head_ogs_lower[:,0]))*0.1 + min(head_ogs_lower[:,0])), "RMSE Anal - Mean: " + str(format(rmse_anal_mean, "10.2E")) + " m")
        ax.text(obs[10], ((max(head_ogs_lower[:,0]) - min(head_ogs_lower[:,0]))*0.15 + min(head_ogs_lower[:,0])), "RMSE Anal - Lower: " + str(format(rmse_anal_lower, "10.2E")) + " m")
        ax.text(obs[10], ((max(head_ogs_lower[:,0]) - min(head_ogs_lower[:,0]))*0.2 + min(head_ogs_lower[:,0])), "RMSE Anal - Upper: " + str(format(rmse_anal_upper, "10.2E")) + " m")
        ax.text(obs[10], ((max(head_ogs_lower[:,0]) - min(head_ogs_lower[:,0]))*0.25 + min(head_ogs_lower[:,0])), "Mean groundwater head: " + str(round(mean_head,2)) + " m")
...
# calculate RMSE
# =============================================================================
rmse_anal_mean = sum(([(a - b)**2 for a, b in zip(head_ogs_obs_mean, head_gw_model)]))/len(head_ogs_obs_mean)
rmse_anal_lower = sum(([(a - b)**2 for a, b in zip(head_ogs_upper[:][0], head_gw_model)]))/len(head_ogs_obs_mean)
rmse_anal_upper = sum(([(a - b)**2 for a, b in zip(head_ogs_lower[:][0], head_gw_model)]))/len(head_ogs_obs_mean)
