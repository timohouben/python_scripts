...
# iteration adds less than this relative value, default: 0.01
convergence = 0.01
# the number of the curve (1st == 1)
recharge_rfd = 1
...
- Time series will be loaded and all necessary parameters just from the ogs files
    and stored in array.
- kf values will be taken from the mmp file and will be stored in the dataframe.
    ...
    if i%slots == rank:
        ...
        for j, (obs_point, obs_loc) in enumerate(zip(obs_point_list, obs_loc_list)):
            ...
            if obs_loc == aquifer_length:
                ...
                marker = ["", ""]
                figtxt = "OGS Input Parameter: S = %1.3e, D = %1.3e" % (
                    S_in,
                    T_in/S_in,
