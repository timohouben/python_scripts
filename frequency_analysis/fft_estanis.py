...
fft_data = np.loadtxt(
    "/Users/houben/PhD/modelling/transect/ogs/confined/transient/rectangular/Groundwater@UFZ/Model_Setup_D_day_EVE/homogeneous/D18-D30/testing2/Groundwater@UFZ_eve_HOMO_276_D_4_results/head_ogs_obs_0400_mean.txt"
)
recharge = np.loadtxt(
    "/Users/houben/PhD/modelling/transect/ogs/confined/transient/rectangular/Groundwater@UFZ/Model_Setup_D_day_EVE/homogeneous/D18-D30/testing2/Groundwater@UFZ_eve_HOMO_276_D_4_results/rfd_curve#1.txt"
)
...
def fft_psd(
    ...
    with open(str(path_to_project) + "PSD_output.txt", "a") as file:
        file.write(
            "date time method T_l[m2/s] kf_l[m/s] Ss_l[1/m] D_l[m2/s] a_l t_l[s] path_to_project observation_point\n"
        )
    file.close()
    ...
    if len(recharge) > len(fft_data):
        print(
            "Your input and output data have a different length. Adjusting to the smaller one by deleting last entries."
        )
    ...
    if method == "scipyfft":
        ...
        power_spectrum_input = (
            abs(scpfft.fft(recharge_detrend)[: len(fft_data_detrend) / 2]) ** 2
        )[1:]
        global power_spectrum_output
        power_spectrum_output = (
            abs(scpfft.fft(fft_data_detrend)[: len(fft_data_detrend) / 2]) ** 2
        )[1:]
        global power_spectrum_result
        power_spectrum_result = power_spectrum_output / power_spectrum_input
        frequency_input = (
            abs(scpfft.fftfreq(len(fft_data_detrend), time_step_size))[
    ...
    ax.set_title(
        "power spectrum density for observation point "
        + str(obs_point)
        + "\n"
        + "method: "
        + str(method)
    ...
    if fit == True:
        ...
        power_spectrum_result_filtered = signal.savgol_filter(
            power_spectrum_result, window_size, 2
        )
        ...
        if a_l == None and t_l == None:
            ...
            if len(power_spectrum_result_filtered) % len(weights_l) != 0:
                for residual in range(
                    len(power_spectrum_result_filtered) % len(weights_l)
                ):
        ...
        for i in range(0, len(frequency_input)):
            line = 1.0 / (
                a_l ** 2 * (1 + ((t_l ** 2) * (frequency_input[i] ** 2)))
            )  # method 1
        ...
        plt.figtext(
            ...
            bbox=dict(
                boxstyle="round", facecolor="#F2F3F4", ec="0.5", pad=0.5, alpha=1
            ),
        )
