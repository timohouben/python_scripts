...
def fft_psd(
    ...
    if method == "scipyffthalf":
        ...
        # =========================================================================
        power_spectrum_input = (
            abs(fftpack.fft(recharge_detrend, len_input)[: len_output / 2]) ** 2
        )[1:]
        power_spectrum_output = (
            abs(fftpack.fft(fft_data_detrend, len_output)[: len_output / 2]) ** 2
        )[1:]
        power_spectrum_result = power_spectrum_output / power_spectrum_input
        frequency_input = (
    ...
    if method == "scipywelch":
        ...
        frequency_input, power_spectrum_input = signal.welch(
            recharge_detrend, sampling_frequency, nperseg=16000, window="hamming"
        )
        frequency_output, power_spectrum_output = signal.welch(
            fft_data_detrend, sampling_frequency, nperseg=16000, window="hamming"
        )
    ...
    if fit == True:
        ...
        if a_d == None and t_d == None and dupuit == True:
            ...
            def dupuit_fit(w_d, a_d, t_d):
                return (1.0 / a_d) ** 2 * (
                    (1.0 / (t_d * w_d))
                    * np.tanh((1 + 1j) * np.sqrt(1.0 / 2 * t_d * w_d)).real
                    * np.tanh((1 - 1j) * np.sqrt(1.0 / 2 * t_d * w_d)).real
                )
