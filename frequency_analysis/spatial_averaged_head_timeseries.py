...
def spatial_averaged_head_timeseries(path_to_multiple_projects, remove_value=None):
    ...
    file_names, obs_names, obs_locs = get_obs(
        path_to_multiple_projects + "/" + project_folder_list[0]
    )

    file_names_txt = [
        "head_ogs_" + obs_names[i] + "_mean.txt" for i, item in enumerate(obs_names)
    ]
    ...
    for i in range(0, len(project_folder_list)):
        ...
        for k in range(0, len(files[i][j])):
            ...
            av_head_timeseries.append(
                integrate.simps(head_loc, obs_locs) / max(obs_locs)
            )
